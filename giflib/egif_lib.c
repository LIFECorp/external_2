...
EGifOpenFileName(const char *FileName,
                 const bool TestExistance, int *Error) {
 ...
 *****************************************************************************/
GifFileType *
EGifOpenFileHandle(int FileHandle, int *Error) {
    ...
    GifFile->UserData = (void *) 0;    /* No user write handle (MRB) */
...
EGifOpen(void *userData,
         OutputFunc writeFunc, int *Error) {
 ...
 *****************************************************************************/
void
EGifSetGifVersion(GifFileType *GifFile,  const bool gif89) {
    if (gif89 != 0)
        strncpy(GifVersionPrefix + GIF_VERSION_POS, "89a", 3);
    else
        strncpy(GifVersionPrefix + GIF_VERSION_POS, "87a", 3);
}
