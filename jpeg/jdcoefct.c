...
decompress_onepass (j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
  ...
  if (cinfo->tile_decode) {
    ...
    if( cinfo->coef->mtk_mt_MCU_buffer[0] ){
      int j =0;
      for (j = 0; j < D_MAX_BLOCKS_IN_MCU; j++) {
        coef->MCU_buffer[j] = coef->pub.mtk_mt_MCU_buffer[j] ;
      }
    }
#endif
...
jinit_d_coef_controller (j_decompress_ptr cinfo, boolean need_full_buffer)
{
  ...
  #ifdef SKIA_MT_LIBJPEG_TILE_DECODE
    memset( cinfo->coef , 0 , SIZEOF(my_coef_controller) );
    coef->pub.my_struct_size = SIZEOF(my_coef_controller) ;
  #endif
