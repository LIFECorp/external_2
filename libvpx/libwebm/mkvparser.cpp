    ...
    Track*& pTrack) const
{
...
#ifndef ANDROID_DEFAULT_CODE
			{
				ALOGE("Tracks::ParseTrackEntry, Track UID (id == 0x33C5) size =%lld not in range (0-8), return error!",size);
				return E_FILE_FORMAT_INVALID;
			}
#else
            return E_FILE_FORMAT_INVALID;
...
#ifndef ANDROID_DEFAULT_CODE
	{
		ALOGE("Tracks::ParseTrackEntry,  there is already a Track with Number(%ld) , return error!",i.number);
		return E_FILE_FORMAT_INVALID;
	}
#else
        return E_FILE_FORMAT_INVALID;
...
#endif
#ifdef MTK_SUBTITLE_SUPPORT
    if ((i.type != Track::kVideo) && (i.type != Track::kAudio) && (i.type != Track::kSubTT))
#else
    ...
    if (i.type == Track::kVideo)
    {
        ...
        if(m_pSegment->m_videotracknumber==0){
			m_pSegment->m_videotracknumber=i.number;
...
#ifdef MTK_SUBTITLE_SUPPORT
    else if (i.type == Track::kSubTT)
    {
        assert(i.type == Track::kSubTT);
        
        SubTtTrack* p = NULL;
        
        status = SubTtTrack::Parse(m_pSegment, i, elem_st, elem_sz, p);
        pTrack = p;
