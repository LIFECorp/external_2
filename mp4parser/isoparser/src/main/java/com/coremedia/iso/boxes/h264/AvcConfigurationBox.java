...
/// M: buffer remain{
		    if (content.remaining() < sequenceParameterSetExtLength) {
			Log.w("mp4parser-AvcConfigurationBoxc", "buffer remaining:"+content.remaining() + "position:"+content.position() + " sps len:"+sequenceParameterSetExtLength);
			sequenceParameterSetExtNALUnit = new byte[content.remaining()];
			/*
			   int pos = content.position();
			   int remain = content.remaining();
			   content.position(pos+ remain);
			   hasExts = false;
			   return;
			 */
