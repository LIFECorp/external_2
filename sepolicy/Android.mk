...
###################################################
# for mtk sepolicy file is high priority than google default, use BOARD_SEPOLICY_REPLACE to set mtk sepolicy priority.

BOARD_SEPOLICY_DIRS +=$(MTK_ROOT_CUSTOM_OUT)/sepolicy
BOARD_SEPOLICY_REPLACE += $(filter $(call find-subdir-assets, $(MTK_ROOT_CUSTOM_OUT)/sepolicy),$(notdir $(wildcard $(LOCAL_PATH)/*)))
BOARD_SEPOLICY_IGNORE+=external/sepolicy/keys.conf

# Add for MTK platform key selection. copy from ./build/core/package.mk 
# If this is not an absolute certificate, assign it to a generic one.
ifeq ($(MTK_SIGNATURE_CUSTOMIZATION),yes)
  KEYS_FILE := release_keys.conf
  ifeq ($(MTK_INTERNAL),yes)
    KEYS_DIR  := $(dir $(DEFAULT_SYSTEM_DEV_CERTIFICATE))common/
  else
    KEYS_DIR  := $(dir $(DEFAULT_SYSTEM_DEV_CERTIFICATE))$(TARGET_PRODUCT)/
  endif
else
  KEYS_DIR  := $(dir $(DEFAULT_SYSTEM_DEV_CERTIFICATE))
  KEYS_FILE := keys.conf
endif
$(info KEYS_DIR: $(KEYS_DIR)keys.conf)


# add other mtk policy to BOARD_SEPOLICY_UNION,otherwise mtk sepolicy can not be built.
################################################################
...
build_policy = $(foreach type, $(1), \
  $(filter-out $(BOARD_SEPOLICY_IGNORE), \
    $(foreach expanded_type, $(filter $(type), $(call find-subdir-assets, $(LOCAL_PATH))), \
    ...
    $(foreach expanded_type, $(filter $(type), $(call find-subdir-assets, $(BOARD_SEPOLICY_DIRS))), \
      $(if $(filter $(expanded_type), $(BOARD_SEPOLICY_REPLACE)), \
        , \
        $(BOARD_SEPOLICY_DIRS)/$(expanded_type) \
      ) \
    ) \
    $(foreach union_policy,$(addprefix $(BOARD_SEPOLICY_DIRS)/,$(filter $(type) , $(call find-subdir-assets,$(BOARD_SEPOLICY_DIRS)))) , \
...
$(info sepolicy_build: $(call build_policy, security_classes initial_sids access_vectors global_macros mls_macros mls policy_capabilities te_macros attributes %.te roles users initial_sid_contexts fs_use genfs_contexts port_contexts))
...
$(info ALL_FC_FILES: $(ALL_FC_FILES))
...
# Build keys.conf
keys_conf.tmp := $(BOARD_SEPOLICY_DIRS)/keys.conf
$(keys_conf.tmp) : .FORCE
	@cp -rf  ./external/sepolicy/$(KEYS_FILE)  $(BOARD_SEPOLICY_DIRS)/keys.conf 
.FORCE:

mac_perms_keys.tmp := $(intermediates)/keys.tmp
$(mac_perms_keys.tmp) : $(keys_conf.tmp) $(call build_policy, keys.conf)
	@mkdir -p $(dir $@)
	$(hide) m4 -s $^ > $@
$(info KEYS_CONF_FILE: $(MTK_SIGNATURE_CUSTOMIZATION) $(MTK_INTERNAL) $(KEYS_FILE))
...
$(LOCAL_BUILT_MODULE) : $(mac_perms_keys.tmp) $(HOST_OUT_EXECUTABLES)/insertkeys.py $(ALL_MAC_PERMS_FILES)
	@mkdir -p $(dir $@)
	$(hide) $(HOST_OUT_EXECUTABLES)/insertkeys.py -t $(TARGET_BUILD_VARIANT) -d $(dir $(KEYS_DIR)) -c $(TOP) $< -o $@ $(ALL_MAC_PERMS_FILES)
