...
void SkDraw::drawBitmap(const SkBitmap& bitmap, const SkMatrix& prematrix,
    ...
    int isD = paint.isDither() ? 1 : 0;
        ...
        if (clipHandlesSprite(*fRC, ix, iy, bitmap)) {
            ...
            if (blitter) {
#if TRACE_DRAW_BITMAP
                SkDebugf("---- sprite case: config=%d [%d %d], device=%d [%d %d], dither=%d,  xfer=%p, alpha=0x%X colorFilter=%p\n",
                        bitmap.config(), bitmap.width(), bitmap.height(), fBitmap->config(), fBitmap->width(), fBitmap->height(), isD,
...
#if DUMP_BITMAPS
                if (access("/data/images", F_OK) == 0)
                    //if(bitmap.width() == 320 && bitmap.height() == 98)
                    //if(fBitmap->width() == 320 && fBitmap->height() == 98)
                    //if( (bitmap.width() == 320 && bitmap.height() == 98) || (fBitmap->width() == 320 && fBitmap->height() == 98))
                {
                    SkMSec now = SkTime::GetMSecs();
                    SkString filename;

                    //src
                    filename.printf("/data/images/%u_sprite_be_%d_D%d.png", now, bitmap.config(), paint.isDither() ? 1 : 0);
                    SkDebugf("save to file  %s", filename.c_str() );
                    saveToFile(filename.c_str(), bitmap);

                    //dst
                    filename.printf("/data/images/%u_sprite_af_%d_D%d.png", now, fBitmap->config(), paint.isDither() ? 1 : 0);
                    SkDebugf("save to file  %s", filename.c_str() );
                    saveToFile(filename.c_str(), *fBitmap);
                }
#endif
...
#if TRACE_DRAW_BITMAP
    SkDebugf("---- MISSING sprite case: config=%d [%d %d], device=%d [%d %d], dither=%d,  xfer=%p, alpha=0x%X colorFilter=%p\n",
            bitmap.config(), bitmap.width(), bitmap.height(), fBitmap->config(), fBitmap->width(), fBitmap->height(), isD,
            paint.getXfermode(), paint.getAlpha(), paint.getColorFilter());
#endif
    ...
    //if( (bitmap.width() == 320 && bitmap.height() == 98) || (fBitmap->width() == 320 && fBitmap->height() == 98))
    {
        ...
        //src
        filename.printf("/data/images/%u_nosp_be_%d_D%d.png", now, bitmap.config(), paint.isDither() ? 1 : 0);
        SkDebugf("save to file  %s", filename.c_str() );
        saveToFile(filename.c_str(), bitmap);

        //dst
        filename.printf("/data/images/%u_nosp_af_%d_D%d.png", now, fBitmap->config(), paint.isDither() ? 1 : 0);
        SkDebugf("save to file  %s", filename.c_str() );
        saveToFile(filename.c_str(), *fBitmap);
    }
#endif
