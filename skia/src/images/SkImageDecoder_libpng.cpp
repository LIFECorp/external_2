...
bool SkPNGImageDecoder::onDecode(SkStream* sk_stream, SkBitmap* decodedBitmap,
    ...
    if (!this->getBitmapConfig(png_ptr, info_ptr, &config, &hasAlpha, &theTranspColor, false)) {
        return false;
    }
    ...
    */
    ...
    if ((SkBitmap::kA8_Config == config || SkBitmap::kIndex8_Config == config)
        ...
        if (bitDepth == 8)
        {
            if (colorType == PNG_COLOR_TYPE_GRAY) {//0
                sc = SkScaledBitmapSampler::kGray;
                srcBytesPerPixel = 1;
            } else if (colorType == PNG_COLOR_TYPE_GRAY_ALPHA) {//4
                sc = SkScaledBitmapSampler::kGrayA;
                srcBytesPerPixel = 2;
            } else if (colorType == PNG_COLOR_TYPE_RGB) {//2
...
bool SkPNGImageDecoder::decodePalette(png_structp png_ptr, png_infop info_ptr,
    ...
    */
    ...
    //work around!!! Some bad image have pixel with index > colorCount, 
    //it will over read the color table and with random value
    if(colorCount < 256)
    {
        colorCount = 256;
    }
...
bool SkPNGImageDecoder::onDecodeSubset(SkBitmap* bm, const SkIRect& region) {
    ...
    if (!this->getBitmapConfig(png_ptr, info_ptr, &config, &hasAlpha, &theTranspColor, true)) {
        return false;
    }
