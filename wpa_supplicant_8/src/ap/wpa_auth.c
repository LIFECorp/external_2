...
static inline int wpa_auth_invoke_alloc_ip(struct wpa_authenticator *wpa_auth)
{
	if (wpa_auth->cb.invoke_alloc_ip == NULL)
		return -1;
	return wpa_auth->cb.invoke_alloc_ip(wpa_auth->cb.ctx);
}

static inline int wpa_auth_get_alloc_ip(struct wpa_authenticator *wpa_auth,
				   struct nfc_ip_alloc *alloc_ip)
{
	if (wpa_auth->cb.get_alloc_ip == NULL)
		return -1;
	return wpa_auth->cb.get_alloc_ip(wpa_auth->cb.ctx, alloc_ip);
}
#endif
...
#ifdef CONFIG_MTK_P2P_NFC_IP_ALLOC
		sm->sta_req_ip_alloc = 0;
		if (kde.ip_req == 1) {
			wpa_printf(MSG_INFO, "WPA: Found IP alloc request in 2/4 handshake");
			sm->sta_req_ip_alloc = 1;
			//wpa_auth_invoke_alloc_ip(sm->wpa_auth);
			//wpa_msg(hapd->msg_ctx, MSG_INFO, P2P_GO_IP_ALLOCATE);
			//a call back in wpa_authenticator may be a better solution
...
SM_STATE(WPA_PTK, PTKINITNEGOTIATING)
{
...
#endif /* CONFIG_IEEE80211R */
#ifdef CONFIG_MTK_P2P_NFC_IP_ALLOC
	//KDE:Type(1) + Length (1) + OUI & Data Type(4) + IPCFG(12)
	if (sm->sta_req_ip_alloc == 1) {
	kde_len += 2 + RSN_SELECTOR_LEN + PMK_LEN * 3;
	}
#endif
	...
	if (sm->sta_req_ip_alloc == 1) {
		...
		wpa_auth_get_alloc_ip(sm->wpa_auth, &alloc_ip);

		os_memcpy(ip_cfg, &alloc_ip.client.s_addr, 4);
		os_memcpy(ip_cfg + 4, &alloc_ip.submask.s_addr, 4);
		os_memcpy(ip_cfg + 8, &alloc_ip.go.s_addr, 4);
		wpa_printf(MSG_INFO, "get_alloc_ip client=0x%08x, mask=0x%08x, go=0x%08x", 
			alloc_ip.client.s_addr, alloc_ip.submask.s_addr, alloc_ip.go.s_addr);
		pos = wpa_add_kde(pos, RSN_KEY_DATA_IP_ALLOC, ip_cfg, 12,
			  	NULL, 0);
