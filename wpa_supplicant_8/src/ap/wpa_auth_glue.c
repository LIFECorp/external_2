...
static int hostapd_wpa_auth_invoke_alloc_ip(void *ctx)
{
	struct hostapd_data *hapd = ctx;

	if (hapd->ip_reserved == 0)
		wpa_msg(hapd->msg_ctx, MSG_INFO, "P2P-GO-IP-ALLOCATE");
	else
		wpa_printf(MSG_DEBUG, "IP reserved before no need to allocate a new IP");
	return 0;
}

static int hostapd_wpa_auth_get_alloc_ip(void *ctx, struct nfc_ip_alloc *alloc_ip)
{
	struct hostapd_data *hapd = ctx;

	hapd->ip_reserved = 0;
	if (hapd->ip_reserved == 1) {
		os_memcpy(alloc_ip, &hapd->alloc_ip, sizeof(struct nfc_ip_alloc));
		os_memset(&hapd->alloc_ip, 0, sizeof(struct nfc_ip_alloc));
		hapd->ip_reserved = 0;
	} else {
		char *client_ip = "192.168.49.40"; 
		char *network_mask   = "255.255.255.0"; 
		char *owner_ip  = "192.168.49.1";
		char client_str[20];

		static unsigned int client_num = 0;
		struct in_addr client;
		struct nfc_ip_alloc  *ip, ip_reserv;
	
		wpa_printf(MSG_INFO, "GET IP INTERNAL\n");
		inet_aton(client_ip, &client);
	
		client_num ++;
		if (client_num > 90)
			client_num = 0;
		client.s_addr += client_num << 24;
		
		os_memset(&ip_reserv, 0, sizeof(ip_reserv));
		ip = &ip_reserv;
		os_strncpy(client_str, inet_ntoa(client), 20);
		inet_aton(client_str, &ip->client);
		inet_aton(network_mask, &ip->submask);
		inet_aton(owner_ip, &ip->go);

		wpa_printf(MSG_INFO, "set_alloc_ip: client ip= 0x%08x, mask=0x%08x, go=0x%08x", 
			ip->client.s_addr, ip->submask.s_addr, ip->go.s_addr);
		
		os_memcpy(alloc_ip, ip, sizeof(struct nfc_ip_alloc));
	}
...
#ifdef CONFIG_MTK_P2P_NFC_IP_ALLOC	
	cb.invoke_alloc_ip = hostapd_wpa_auth_invoke_alloc_ip;
	cb.get_alloc_ip = hostapd_wpa_auth_get_alloc_ip;
#endif
#ifdef CONFIG_IEEE80211R
