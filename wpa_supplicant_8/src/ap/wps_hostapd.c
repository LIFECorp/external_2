...
int wpa_supplicant_ap_set_nfc_password_token(struct hostapd_data *hapd, struct wpabuf *wps_nfc_dh_privkey, 
	struct wpabuf *wps_nfc_dh_pubkey)
{
	struct wps_context *wps = hapd->wps;
	char pw[32 * 2 + 1];

	wpa_printf(MSG_INFO, "add nfc_dh_pubkey");
	if (wps == NULL) {
		wpa_printf(MSG_INFO, "hapd->wps was NULL");
		return -1;
	}
	dh5_free(wps->dh_ctx);
	wpabuf_free(wps->dh_pubkey);
	wpabuf_free(wps->dh_privkey);
	wps->dh_privkey = wpabuf_dup(wps_nfc_dh_privkey);
	wps->dh_pubkey = wpabuf_dup(wps_nfc_dh_pubkey);
	if (wps->dh_privkey == NULL || wps->dh_pubkey == NULL) {
	wpa_printf(MSG_INFO, "no nfc_dh_pubkey");
		wps->dh_ctx = NULL;
		wpabuf_free(wps->dh_pubkey);
		wps->dh_pubkey = NULL;
		wpabuf_free(wps->dh_privkey);
	...
	if (wps->dh_ctx == NULL)
		return -1;	
	wpa_printf(MSG_DEBUG, "p2p_nfc_add_password_token wps->dh_privkey=0x%x", wps->dh_privkey);		
	return 0;
	
}

int wpa_supplicant_ap_wps_nfc_password_token(struct hostapd_data *hapd, const u8 *bssid,
			     	const u8 *oob_dev_password, size_t oob_dev_password_len)
{
#if 0
	struct wps_parse_attr attr;
	...
	attr.oob_dev_password = oob_dev_password;
	attr.oob_dev_password_len = oob_dev_password_len;

	if (attr.oob_dev_password && wpa_s->ap_iface)
		return hostapd_wps_add_nfc_password_token(wpa_s->ap_iface->bss[0], &attr);
#endif		
	struct wps_nfc_password_token_data data;

	wpa_hexdump(MSG_DEBUG, "WPS: AP NFC PASSWORD TOKEN", oob_dev_password, oob_dev_password_len);
	data.oob_dev_pw = oob_dev_password;
	data.oob_dev_pw_len = oob_dev_password_len;
	data.added = 0;
	if (hostapd_wps_for_each(hapd, wps_add_nfc_password_token, &data) < 0)
		return -1;
	return data.added ? 0 : -1;
