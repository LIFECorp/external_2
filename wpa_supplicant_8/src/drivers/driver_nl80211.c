...
static int nl80211_set_wowlan_support(struct i802_bss *bss, 
	enum nl80211_iftype nlmode)
{
	struct wpa_driver_nl80211_data * drv = bss->drv;
	struct nl_msg* msg = NULL;
	struct nlattr *wowlan = NULL;
	int ret = -1;
	msg = nlmsg_alloc();
	if (!msg)
		return -ENOMEM;
	/*fill cmd base*/
	nl80211_cmd(bss->drv, msg, 0, NL80211_CMD_SET_WOWLAN);
	wpa_printf(MSG_DEBUG, "set wowlan for if %s, index %d\n", 
		bss->ifname, bss->ifindex);
	NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, bss->ifindex);
	/*fill wowlan atrri*/
	...
	ret = send_and_recv_msgs(drv, msg, NULL, NULL);
	if (ret)
		wpa_printf(MSG_DEBUG, "nl80211: set_wowlan failed; err=%d %s)",
			   ret, strerror(-ret));
	return ret;
nla_put_failure:
	nlmsg_free(msg);
				       ...
				       enum nl80211_iftype nlmode)
{
	...
	/*add by mtk80707, to set wowlan*/
	res =  nl80211_set_wowlan_support(bss, nlmode);
	if (ret == 0)
		wpa_printf(MSG_DEBUG, "set wowlan ok\n");
	/*end by mtk80707*/
