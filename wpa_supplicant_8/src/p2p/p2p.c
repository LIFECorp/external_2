...
#ifdef CONFIG_MTK_P2P_NFC
extern int p2p_nfc_update_password_token(struct wpa_supplicant *wpa_s, u8 *oob_dev_pw,
					 size_t oob_dev_pw_len);	
int p2p_add_device_nfc(struct wpa_supplicant *wpa_s, struct p2p_data *p2p, 
			const struct wpabuf *p2p_nfc)
{
	...
	os_memset(&msg, 0, sizeof(msg));
	if (p2p_parse_p2p_nfc(p2p_nfc, &msg)) {
		p2p_dbg(p2p, "P2P: Failed to parse P2P NFC TAG for a device entry");
		p2p_parse_free(&msg);
		return -1;
	}
	...
	if (msg.p2p_device_addr)
		...
		p2p_dbg(p2p, "P2P: Ignore scan data without P2P Device Info or "
			"P2P Device Id");
		p2p_parse_free(&msg);
		return -1;
	}
	...
	if (!is_zero_ether_addr(p2p->peer_filter) &&
	    os_memcmp(p2p_dev_addr, p2p->peer_filter, ETH_ALEN) != 0) {
		p2p_dbg(p2p, "P2P: Do not add peer "
	...
	dev->flags &= ~(P2P_DEV_PROBE_REQ_ONLY | P2P_DEV_GROUP_CLIENT_ONLY);
	dev->flags |= P2P_DEV_WITH_NFC;
	dev->wps_method = WPS_PIN_NFC;

  if (msg.oob_go_neg_channel) {
	  int listen_freq;
	  listen_freq = p2p_channel_to_freq(msg.oob_go_neg_channel[3],
					 msg.oob_go_neg_channel[4]);
	  if (listen_freq < 0) {
		  p2p_dbg(p2p, "P2P: Unknown peer OOB GO NEG channel: "
			  "country=%c%c(0x%02x) reg_class=%u channel=%u",
			  msg.oob_go_neg_channel[0],
			  msg.oob_go_neg_channel[1],
			  msg.oob_go_neg_channel[2],
			  msg.oob_go_neg_channel[3],
			  msg.oob_go_neg_channel[4]);
		  wpa_printf(MSG_DEBUG, "Default freq 2412 is used for listen channel");
		  dev->listen_freq = 2412;
  
	  } else {
		  p2p_dbg(p2p, "P2P: Update "
			  "peer " MACSTR " Listen channel: %u -> %u MHz",
			  MAC2STR(dev->info.p2p_device_addr),
			  dev->listen_freq, listen_freq);
		  dev->listen_freq = listen_freq;
	  }
	  if (msg.oob_role_indication)
		  dev->oob_role_indication = msg.oob_role_indication[0];
  }
	...
	if (ssid) {
		...
		wpa_printf(MSG_DEBUG, "P2P: Group was formated...LOCAL=%s", 
			local_role==2? "GO":"GC");
	}	
	
  if (msg.oob_dev_password_len > 0) {
  	struct wpabuf *pw;	

	  pw = wpabuf_alloc(msg.oob_dev_password_len);
		if (pw == NULL)
			return -1;
		wpabuf_put_data(pw, msg.oob_dev_password, msg.oob_dev_password_len);
		// The Local P2P NFC token should be built in advance
		if (p2p->p2p_nfc_local_type == 0){
			struct wpabuf *temp_buf;	
	  	
	  	temp_buf = wpas_p2p_nfc_req_token(wpa_s);
	  	wpabuf_free(temp_buf);
	 	}	 		
		if (p2p->p2p_nfc_peer_pw_token)	
			wpabuf_free(p2p->p2p_nfc_peer_pw_token);
		p2p->p2p_nfc_peer_pw_token = pw;
		//Handle tag according to local role
		//P2P: Update password token and only Select password is used
		//GC: Do nothing
		//GO: Add peer's password token
		switch (local_role) {
			case 0:
				if (p2p->p2p_nfc_handover == 0) {
					if ((dev->oob_role_indication == 0) || (dev->oob_role_indication == 2))
					p2p_nfc_update_password_token(wpa_s, wpabuf_head(pw), wpabuf_len(pw));
				}
				break;
			case 1:
				break;
			case 2:	
                                if(wpa_s->ap_iface){
				    wpa_supplicant_ap_wps_nfc_password_token(wpa_s->ap_iface->bss[0], p2p_dev_addr,
				...
				wpa_printf(MSG_DEBUG, "P2P: GO add new device's password token");
			break;	
		}
//		 p2p->p2p_nfc_handover = 0;
#if 0		
		if (dev->oob_role_indication == 0) {
			if (local_role == 0) {
	  p2p_nfc_update_password_token(wpa_s, wpabuf_head(pw), wpabuf_len(pw)); 	
			} else if (local_role == 2) {
			    if(wpa_s->ap_iface){
				    wpa_supplicant_ap_wps_nfc_password_token(wpa_s->ap_iface->bss[0], p2p_dev_addr,
					      wpabuf_head(p2p->p2p_nfc_peer_pw_token), wpabuf_len(p2p->p2p_nfc_peer_pw_token));
                            }
	...
	if (msg.group_id) {
		os_memcpy(dev->member_in_go_iface, msg.group_id, ETH_ALEN);
	}
	...
	if (dev->flags & P2P_DEV_REPORTED)
		return 0;
#endif
	p2p_dbg(p2p, "P2P: Peer found with NFC tag");
	if (dev->flags & P2P_DEV_USER_REJECTED) {
		p2p_dbg(p2p, "P2P: Do not report rejected device");
		return 0;
	}
	dev->info.oob_role_indication = dev->oob_role_indication;
	dev->info.flags= dev->flags;
...
#ifdef CONFIG_MTK_P2P_NFC
	if (msg->dev_password_id > 0x10) {
		dev->oob_role_indication = 0;		
		dev->flags |= P2P_DEV_WITH_NFC;
		dev->wps_method = WPS_PIN_NFC;
		dev->info.oob_role_indication = dev->oob_role_indication;
		dev->info.flags= dev->flags;
