...
void p2p_buf_add_oob_go_neg_channel(struct wpabuf *buf, const char *country,
				u8 reg_class, u8 channel, u8 go_indicate)
{
	/* Listen Channel */
	...
	wpabuf_put_u8(buf, reg_class); /* Regulatory Class */
	wpabuf_put_u8(buf, channel); /* Channel Number */
	wpabuf_put_u8(buf, go_indicate); /* Channel Number */
	wpa_printf(MSG_DEBUG, "P2P: * OOB GO NEG Channel: Regulatory Class %u "
		   "Channel %u", reg_class, channel);
}


void p2p_buf_add_p2p_version(struct wpabuf *buf)
{
	wpa_printf(MSG_DEBUG, "P2P:  * P2P Version ");
	wpabuf_put_u8(buf, P2P_ATTR_P2P_VERSION);
	wpabuf_put_le16(buf, 4);
	wpabuf_put_be24(buf, OUI_WFA);
    wpabuf_put_u8(buf, P2P_OUI_TYPE);
}
void p2p_buf_add_peer_device_info(struct wpabuf *buf, struct p2p_data *p2p,
			     struct p2p_device *peer)
{
	...
	/* P2P Device address */
	wpabuf_put_data(buf, info->p2p_device_addr, ETH_ALEN);

	/* Config Methods */
	wpabuf_put_be16(buf, info->config_methods);

	/* Primary Device Type */
	wpabuf_put_data(buf, info->pri_dev_type,
			sizeof(info->pri_dev_type));
...
#endif
	/* Number of Secondary Device Types */
	wpabuf_put_u8(buf, info->wps_sec_dev_type_list_len / WPS_DEV_TYPE_LEN);

	/* Secondary Device Type List */
	wpabuf_put_data(buf, info->wps_sec_dev_type_list, info->wps_sec_dev_type_list_len);

	/* Device Name */
	nlen = os_strlen(info->device_name);
	wpabuf_put_be16(buf, ATTR_DEV_NAME);
	wpabuf_put_be16(buf, nlen);
	wpabuf_put_data(buf, info->device_name, nlen);
