...
#ifdef CONFIG_MTK_P2P_NFC	
	if (peer->wps_method == WPS_PIN_NFC) {
		p2p_build_wps_ie(p2p, buf, p2p->p2p_nfc_dev_pw_id, 0);		
	} else 
#endif	
...
#ifdef CONFIG_MTK_P2P_NFC	
		else if (dev->wps_method == WPS_PIN_NFC)
			config_method = WPS_CONFIG_NFC_INTERFACE;
#endif /* CONFIG_MTK_P2P_NFC */	
		else
			return -1;
...
#ifdef CONFIG_MTK_P2P_NFC	
	if (peer->wps_method == WPS_PIN_NFC) {
		p2p_build_wps_ie(p2p, buf, (peer)? p2p->p2p_nfc_dev_pw_id :
			WPS_NOT_READY, 0);		
	} else 
#endif	
...
#ifdef CONFIG_MTK_P2P_NFC
	else if (msg.dev_password_id > 0x10)
		p2p_add_dev_info(p2p, sa, dev, &msg);
#endif
...
*/		
		...
		case DEV_PW_PUSHBUTTON:
			...
			break;
#ifdef CONFIG_MTK_P2P_NFC_RESERVED				
		case DEV_PW_NFC_CONNECTION_HANDOVER:
			wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
				"P2P: Peer using NFC");
			if (dev->wps_method != WPS_PIN_NFC) {
				wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
		...
		default:
#ifdef CONFIG_MTK_P2P_NFC
			p2p_dbg(p2p, "P2P: Peer PW ID=%02X, local PW ID=%02X", msg.dev_password_id, p2p->p2p_nfc_dev_pw_id);				
			if ((dev->wps_method == WPS_PIN_NFC) && (msg.dev_password_id == p2p->p2p_nfc_dev_pw_id)) {
				p2p_dbg(p2p, "P2P: Peer using NFC");				
			} else {				
				p2p_dbg(p2p, "P2P: Unsupported Device Password ID %d",
					msg.dev_password_id);
				status = P2P_SC_FAIL_INCOMPATIBLE_PROV_METHOD;
				goto fail;
			}
