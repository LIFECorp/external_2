			       ...
			       struct p2p_message *msg)
{
	...
	case P2P_ATTR_OOB_GO_NEG_CHANNEL:
		if (len == 0) {
			wpa_printf(MSG_DEBUG, "P2P: * OOB GO NEG Channel: Ignore "
				   "null channel");
			break;
		}
		if (len < 6) {
			wpa_printf(MSG_DEBUG, "P2P: Too short OOB GO NEG Channel "
				   "attribute (length %d)", len);
			return -1;
		}
		...
		wpa_printf(MSG_DEBUG, "P2P: * OOB GO NEG Channel: "
			   ...
			   "Class %d Channel Number %d", data[0], data[1],
			   data[2], data[3], data[4]);
		msg->oob_role_indication = data + 5;
		break;
#endif		
...
#ifdef CONFIG_MTK_P2P_NFC
  if (attr.oob_dev_password_len > 0) {
...
#endif /* CONFIG_MTK_WFD */
#ifdef CONFIG_MTK_P2P_NFC
int p2p_parse_p2p_nfc(const struct wpabuf *buf, struct p2p_message *msg)
{
	const u8 *pos = wpabuf_head_u8(buf);
	const u8 *end = pos + wpabuf_len(buf);
	u16 attr_len;

	wpa_printf(MSG_DEBUG, "P2P: Parsing P2P NFC tag");

	if (pos < end) {
		if (pos + 2 >= end) {
			wpa_printf(MSG_DEBUG, "P2P: Invalid WPS attributes length");
			return -1;
		}
		attr_len = WPA_GET_BE16(pos);
	    wpa_printf(MSG_INFO, "P2P: WPS attributes len=%d", attr_len);
		if (attr_len > 500) {
	        wpa_printf(MSG_ERROR, "P2P: Unreasonable WPS len=%d", attr_len);
            return -1;
		}
	...
	if (pos < end) {
		if (pos + 2 >= end) {
			wpa_printf(MSG_DEBUG, "P2P: Invalid P2P attributes length");
			return -1;
		}
		attr_len = WPA_GET_BE16(pos);
	    wpa_printf(MSG_INFO, "P2P: P2P attributes len=%d", attr_len);
		if (attr_len > 500) {
	        wpa_printf(MSG_ERROR, "P2P: Unreasonable P2P len=%d", attr_len);
            return -1;
		}
