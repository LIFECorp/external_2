...
{
/* Google AOSP block the freq [5170, 5745) to act as p2p operating channel. Turnkey code adds them back
  because of P2P SIGMA(channel 36), and ETC.
*/
#if defined (CONFIG_MTK_P2P) || defined (CONFIG_MTK_P2P_SIGMA)
	wpa_printf(MSG_DEBUG, "Bring back 5G channels blocks by AOSP 4.4 supplicant");
	return 0;
#else
