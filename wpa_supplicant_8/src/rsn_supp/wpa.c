...
#ifdef CONFIG_MTK_P2P_NFC_IP_ALLOC
	ip_alloc_need = wpa_sm_check_ip_alloc_need(sm);
	rbuf = wpa_sm_alloc_eapol(sm, IEEE802_1X_TYPE_EAPOL_KEY,
				  NULL, sizeof(*reply) + wpa_ie_len + ((ip_alloc_need == 1)? 7: 0),
				  &rlen, (void *) &reply);
#else
	rbuf = wpa_sm_alloc_eapol(sm, IEEE802_1X_TYPE_EAPOL_KEY,
...
#ifdef CONFIG_MTK_P2P_NFC_IP_ALLOC
	if (ip_alloc_need == 1) {	
		pos = reply + 1; 			//point to key_data
		pos += wpa_ie_len;
		*pos++ = WLAN_EID_VENDOR_SPECIFIC;
		*pos++ = RSN_SELECTOR_LEN + 1;
		RSN_SELECTOR_PUT(pos, RSN_KEY_DATA_IP_REQ);
		pos += RSN_SELECTOR_LEN;
		*pos = 1;				//1:request ip address
		WPA_PUT_BE16(reply->key_data_length, wpa_ie_len + 7);
	}
#endif	
	os_free(rsn_ie_buf);
...
#ifdef CONFIG_MTK_P2P_NFC_IP_ALLOC
	if (ie.ip_alloc ) {
		if (ie.ip_alloc_len == 12) {
			struct nfc_ip_alloc alloc_ip;
			char client[20], mask[20], go[20];

			os_memcpy(&alloc_ip.client.s_addr, ie.ip_alloc, 4);
			os_memcpy(&alloc_ip.submask.s_addr, ie.ip_alloc + 4, 4);
			os_memcpy(&alloc_ip.go.s_addr, ie.ip_alloc + 8, 4);	
			wpa_hexdump(MSG_DEBUG, "WPA: ie.ip_alloc:",
					ie.ip_alloc, ie.ip_alloc_len);		
			os_strncpy(client, inet_ntoa(alloc_ip.client),20);
			os_strncpy(mask, inet_ntoa(alloc_ip.submask),20);
			os_strncpy(go, inet_ntoa(alloc_ip.go),20);
			wpa_msg(sm->ctx->msg_ctx, MSG_INFO,
				"P2P-GC-IP-GET client_ip=%s sub_mask=%s go_ip=%s",
				client, mask, go);
		}
	}
