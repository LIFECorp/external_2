...
static char wifi_p2p_handover_type[] = "application/vnd.wfa.p2p";
...
#ifdef CONFIG_MTK_P2P_NFC
static int wifi_p2p_filter(struct ndef_record *record)
{
	if (record->type_length != os_strlen(wifi_p2p_handover_type))
		return 0;
	if (os_memcmp(record->type, wifi_p2p_handover_type,
		      os_strlen(wifi_p2p_handover_type)) != 0)
		return 0;
	return 1;
}

struct wpabuf * ndef_build_wifi_p2p(const struct wpabuf *buf)
{
	return ndef_build_record(FLAG_MESSAGE_BEGIN | FLAG_MESSAGE_END |
				 FLAG_TNF_RFC2046, wifi_p2p_handover_type,
				 os_strlen(wifi_p2p_handover_type), "0", 1, buf);
}
struct wpabuf * ndef_parse_wifi_p2p(const struct wpabuf *buf)
{
	return ndef_parse_records(buf, wifi_p2p_filter);
}
