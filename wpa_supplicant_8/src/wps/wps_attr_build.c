...
int wps_build_public_key(struct wps_data *wps, struct wpabuf *msg)
{
	...
	wpa_printf(MSG_DEBUG, "dev_pw_id = %d, wps->wps->dh_privkey=0x%x", wps->dev_pw_id, wps->wps->dh_privkey);
	...
	if (wps->dev_pw_id != DEV_PW_DEFAULT && wps->wps->dh_privkey &&
		...
		if(wps->dh_ctx == NULL){
            if(wps->dh_privkey == NULL)
                wpa_printf(MSG_DEBUG, "Maggie: wps->dh_privkey = NULL");  
            if(pubkey == NULL)
                wpa_printf(MSG_DEBUG, "Maggie: pubkey = NULL"); 
            if(wps->dh_privkey && pubkey)
                wpa_printf(MSG_DEBUG, "Maggie: dh5_init_fixed"); 
    ...
    if(wps->dh_ctx == NULL){
        wpa_printf(MSG_DEBUG, "WPS: Failed to initialize "
			   "Diffie-Hellman handshake (!wps->dh_ctx)");
		wpabuf_free(pubkey);
		return -1;
    }
    ...
    if(wps->dh_privkey == NULL){
        wpa_printf(MSG_DEBUG, "WPS: Failed to initialize "
			   "Diffie-Hellman handshake (!wps->dh_privkey)");
		wpabuf_free(pubkey);
		return -1;
    }
    ...
    if(pubkey == NULL){
        wpa_printf(MSG_DEBUG, "WPS: Failed to initialize "
			   "Diffie-Hellman handshake (!pubkey)");
		wpabuf_free(pubkey);
		return -1;
    }
#endif /* CONFIG_MTK_WPS_NFC */
...
int wps_build_ssid(struct wpabuf *msg,
			       const u8 *ssid, size_t ssid_len)
{
	wpa_printf(MSG_DEBUG, "WPS:  * SSID");
	wpa_hexdump_ascii(MSG_DEBUG, "WPS: SSID",
			  ssid, ssid_len);
	wpabuf_put_be16(msg, ATTR_SSID);
	wpabuf_put_be16(msg, ssid_len);
	wpabuf_put_data(msg, ssid, ssid_len);
	return 0;
}


int wps_build_ap_channel(struct wpabuf *msg,
				   int ap_channel)
{
    wpa_printf(MSG_DEBUG, "WPS:  * AP Channel (%d)", ap_channel);
...
int wps_build_handover_dev_pw(struct wpabuf *msg, u16 dev_pw_id,
			 ...
			 size_t dev_pw_len)
{
    ...
    wpa_printf(MSG_DEBUG, "WPS: Device Password ID (%d)", dev_pw_id);
...
int wps_build_wps_state(struct wpabuf *msg, u8 state)
{
	wpa_printf(MSG_DEBUG, "WPS:  * WPS STATE (%d)", state);
	wpabuf_put_be16(msg, ATTR_WPS_STATE);
	wpabuf_put_be16(msg, 1);
	wpabuf_put_u8(msg, state);//WPS_STATE_NOT_CONFIGURED);
	return 0;
}

int wps_build_selected_reg_config_methods(struct wpabuf *msg, u16 method)
{
	wpa_printf(MSG_DEBUG, "WPS:  * SEL REG CONFIG METHOD (%d)", method);
	wpabuf_put_be16(msg, ATTR_SELECTED_REGISTRAR_CONFIG_METHODS);
	wpabuf_put_be16(msg, 2);
	wpabuf_put_be16(msg, method);
	return 0;
}
