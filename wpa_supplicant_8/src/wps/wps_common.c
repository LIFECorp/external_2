...
int wps_parse_handover_dev_pwd(struct wps_context *wps,
				 struct wpabuf *data)
{
...
struct wpabuf * wps_build_nfc_handover_req(const u8 *uuid, u16 dev_pw_id,
				       const struct wpabuf *pubkey,
				       const struct wpabuf *dev_pw)
{
	struct wpabuf *data;
    ...
    wpabuf_put_be16(data, 0);
	if (wps_build_handover_dev_pw(data, dev_pw_id, pubkey,
				 NULL, 0) ||
	    wps_build_uuid_e(data, uuid) ||
	    wps_build_wfa_ext(data, 0, NULL, 0)) {
		wpa_printf(MSG_ERROR, "WPS: Failed to build NFC handover "
			   "request");
		wpabuf_free(data);
		return NULL;
	}
    WPA_PUT_BE16((u8 *) wpabuf_head(data), wpabuf_len(data) - 2);
	wpa_printf(MSG_INFO, "WPS: WPS attributes len=%d", wpabuf_len(data) - 2);

	return data;
}


struct wpabuf * wps_build_nfc_handover_sel(const u8 *ssid, size_t ssid_len,
                       struct wps_device_data *dev, int ap_channel, 
                       const u8 *bssid, u16 dev_pw_id,
				       const struct wpabuf *pubkey,
				       const struct wpabuf *dev_pw)
{
	struct wpabuf *data;
	...
	if (wps_build_handover_dev_pw(data, dev_pw_id, pubkey,
				 NULL, 0) ||
	    wps_build_ssid(data, ssid, ssid_len) ||
	    wps_build_rf_bands(dev, data) ||
	    wps_build_ap_channel(data, ap_channel) ||
	    wps_build_mac_addr(data, bssid) ||
	    wps_build_wfa_ext(data, 0, NULL, 0)) {
		wpa_printf(MSG_ERROR, "WPS: Failed to build NFC handover "
			   "select");
		wpabuf_free(data);
...
struct wpabuf * wps_nfc_handover_req_gen(const u8 *uuid, int *id, 
				  ...
				  struct wpabuf **dev_pw)
{
    ...
    os_memset(wpabuf_put(pw, WPS_OOB_DEVICE_PASSWORD_LEN), 0, 
                      WPS_OOB_DEVICE_PASSWORD_LEN);
	...
	*id = DEV_PW_NFC_CONNECTION_HANDOVER;
	wpabuf_free(*pubkey);
	*pubkey = pub;
	wpabuf_free(*privkey);
	*privkey = priv;
	...
	*dev_pw = pw;
	...
	ret = wps_build_nfc_handover_req(uuid, *id, *pubkey, *dev_pw);
    if (ret) {
		struct wpabuf *tmp;
		tmp = ndef_build_wifi(ret);
		wpabuf_free(ret);
...
struct wpabuf * wps_nfc_handover_sel_gen(const u8 *ssid, size_t ssid_len,
                  struct wps_device_data *dev, int ap_channel, 
				  ...
				  struct wpabuf **dev_pw)
{
	...
	*dev_pw = pw;
	...
	ret = wps_build_nfc_handover_sel(ssid, ssid_len, dev, 
               ap_channel, bssid, *id, *pubkey, *dev_pw);
    if (ret) {
		struct wpabuf *tmp;
		tmp = ndef_build_wifi(ret);
		wpabuf_free(ret);
