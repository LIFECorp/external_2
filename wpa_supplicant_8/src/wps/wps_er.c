...
int wps_er_nfc_password_token(struct wps_context *wps, struct wpabuf *wps_nfc_dh_privkey, 
	struct wpabuf *wps_nfc_dh_pubkey)
{
	char pw[32 * 2 + 1];

	wpa_printf(MSG_INFO, "WPS ER: Add select message password token");
	if (wps == NULL) {
		wpa_printf(MSG_INFO, "WPS ER: wpa_s->wps = NULL");
		return -1;
	}
	dh5_free(wps->dh_ctx);
	...
	if (wps->dh_privkey == NULL || wps->dh_pubkey == NULL) {
        wpa_printf(MSG_INFO, "WPS ER: wps->dh_privkey or  wps->dh_pubkey = NULL");
		wps->dh_ctx = NULL;
		wpabuf_free(wps->dh_pubkey);
		wps->dh_pubkey = NULL;
		wpabuf_free(wps->dh_privkey);
	...
	if (wps->dh_ctx == NULL)
		return -1;	
	wpa_printf(MSG_DEBUG, "WPS ER: DH Private Key =0x%x", wps->dh_privkey);		
	return 0;
