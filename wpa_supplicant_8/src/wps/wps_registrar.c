						      ...
						      u16 pw_id)
{
	...
	dl_list_for_each(token, tokens, struct wps_nfc_pw_token, list) {
		if (token == NULL)
			return NULL;
		wpa_printf(MSG_DEBUG, "P2P_NFC:target_id = %d, token->pw_id =%d", pw_id, token->pw_id);
...
#ifdef CONFIG_MTK_WPS_NFC
    char uuid_str[100];
	uuid_bin2str(attr.uuid_e, uuid_str, sizeof(uuid_str));
    wpa_msg(reg->cb_ctx, MSG_INFO, WPS_EVENT_ER_ENROLLEE_ADD_PBC "%s " MACSTR,
		uuid_str, MAC2STR(addr));
...
extern int p2p_nfc_password_valid;
extern int p2p_nfc_password_len;
extern u8 p2p_nfc_password[32];
...
static int wps_get_dev_password(struct wps_data *wps)
{
	const u8 *pin;
	size_t pin_len = 0;
#ifdef CONVERT_NFC_HEX_PW_TO_ASCII	
    u8 nfc_pw[65];
#endif
...
#ifdef CONFIG_MTK_P2P_NFC
	if (pin == NULL && p2p_nfc_password_valid) {
		wpa_printf(MSG_DEBUG, "WPS: Use OOB Device Password from NFC");
		pin = p2p_nfc_password;
		pin_len = p2p_nfc_password_len;
		wpa_printf(MSG_INFO, "WPS: PIN=[%02x][%02x][%02x][%02x]... PIN_LEN=%d",
			pin[0], pin[1], pin[2], pin[3], pin_len);
	}
#endif /* CONFIG_MTK_P2P_NFC */

#ifdef CONFIG_MTK_WPS_NFC
    if (pin == NULL && wps->dev_pw_id == DEV_PW_NFC_CONNECTION_HANDOVER) {
        wpa_printf(MSG_DEBUG, "WPS: Device Password is set 0 to NFC Handover.");
...
static void wps_cred_update_nfc(struct wps_credential *dst,
...
static struct wpabuf * wps_build_m2_with_config(struct wps_data *wps)
{
	struct wpabuf *msg, *plain;
	...
	wpa_printf(MSG_DEBUG, "WPS: Building Message M2 with ConfigData");
	msg = wpabuf_alloc(1000);
	if (msg == NULL)
		return NULL;
    ...
    if (wps->new_ap_settings) {
		wpa_printf(MSG_INFO, "WPS: Update AP configuration based on "
			   "new settings");
		wps_cred_update_nfc(&wps->cred, wps->new_ap_settings);
        os_memcpy(&wps->cred.mac_addr, wps->mac_addr_e, ETH_ALEN);
	}
...
#ifdef CONFIG_MTK_P2P_NFC
		/* Only for NFC handover */
        if(wps->dev_pw_id == DEV_PW_NFC_CONNECTION_HANDOVER)
			msg = wps_build_m2_with_config(wps);
		else
#endif				
...
#ifdef CONFIG_WPS_NFC
#ifdef CONFIG_MTK_P2P_NFC
    wpa_printf(MSG_DEBUG, "Maggie: M1 Device Password ID %d",
			   wps->dev_pw_id);
	if (wps->dev_pw_id >= 0x10 || 
        	wps->dev_pw_id == DEV_PW_NFC_CONNECTION_HANDOVER) {
#else
		...
		if (token) {
...
#ifdef CONFIG_MTK_P2P_NFC
			if (os_memcmp(P2P_NFC_LOCAL_PUBLIC_KEY_HASH, wps->nfc_pw_token->pubkey_hash,
				      strlen(P2P_NFC_LOCAL_PUBLIC_KEY_HASH)) == 0) {
				wpa_printf(MSG_INFO, "WPS: No Peer's Public Key hash, Skip hash checking");
			} else
#endif
			if (os_memcmp(hash, wps->nfc_pw_token->pubkey_hash,
				...
				wpa_printf(MSG_ERROR, "WPS: Public Key hash "
					   "mismatch");
#ifdef CONFIG_MTK_P2P_NFC
				/* Maggie : WPS_v2.0.2.1-rev7 add config error code
				return WPS_FAILURE; */
				wps->config_error = WPS_CFG_PUBLIC_KEY_HASH_MISMATCH;
				wps->state = SEND_M2D;
				wps_fail_event(wps->wps, WPS_M1, wps->config_error,
...
#ifdef CONFIG_MTK_P2P_NFC
		else if(wps->dev_pw_id == DEV_PW_NFC_CONNECTION_HANDOVER){
			wpa_printf(MSG_ERROR, "WPS: Not found matching NFC "
				   "Password Token");
			wpa_printf(MSG_ERROR, "WPS: Public Key hash "
					   "mismatch");
				/* Maggie : WPS_v2.0.2.1-rev7 add config error code
