...
struct wpabuf * wpa_supplicant_ap_wps_nfc_config_token(struct wpa_supplicant *wpa_s, 
					     int ndef)
{
	int ret, ret_len = 0;
	...
	return hostapd_wps_nfc_config_token(wpa_s->ap_iface->bss[0], ndef);
}
...
int wpas_wps_ap_set_alloc_ip(struct wpa_supplicant *wpa_s, const char *client,
			const char *mask, const char *go)
{
	struct hostapd_data *hapd;
	struct nfc_ip_alloc  *ip;
	...
	inet_aton(client, &ip->client);
	inet_aton(mask, &ip->submask);
	inet_aton(go, &ip->go);
	hapd->ip_reserved = 1;
