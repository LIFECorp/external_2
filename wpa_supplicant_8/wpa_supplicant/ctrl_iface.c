					 ...
					 char *cmd)
{
	...
	*value++ = '\0';
	...
	if (os_strcasecmp(cmd, "p2p_go_intent") == 0) {
		wpa_s->force_p2p_go_intent = atoi(value);
	}
#endif	
					     ...
					     char *cmd)
{
	...
	if (wpa_s->ap_iface) 
		return wpas_p2p_go_wps_nfc(wpa_s);	
#endif /* CONFIG_MTK_P2P_NFC */
#ifdef CONFIG_MTK_WPS_NFC
	return wpas_wps_start_nfc(wpa_s, _bssid, 0);
#else
...
#ifdef CONFIG_MTK_WPS_NFC
static int wpa_supplicant_ctrl_iface_wps_nfc_cfg(
	struct wpa_supplicant *wpa_s, char *cmd)
{
    ...
    *value++ = '\0';
    ...
    wpa_printf(MSG_DEBUG, "CTRL_IFACE: WPS_NFC_CFG name='%s' value=%s",
           cmd, value);

    wpa_s->fgpubkey = 0;

    if (os_strncmp(cmd, "pubkey", os_strlen("pubkey"))==0) {
        wpa_printf(MSG_INFO, "pubkey=%d", atoi(value));
        wpa_s->fgpubkey = atoi(value);
    }
    else {
        goto fail;
    }
...
#ifdef CONFIG_MTK_P2P_NFC
static int wpa_supplicant_ctrl_iface_wps_nfc_configuration_token(
	struct wpa_supplicant *wpa_s, char *cmd, char *reply, size_t max_len)
{
	int ndef;
	struct wpabuf *buf;
	...
	buf = wpa_supplicant_ap_wps_nfc_config_token(wpa_s, ndef);
	if (buf == NULL)
		return -1;
...
static int wpa_supplicant_ctrl_iface_p2p_nfc_request_token(
	struct wpa_supplicant *wpa_s, char *cmd, char *reply, size_t max_len)
{
	struct wpabuf *buf;
	int res;

	buf = wpas_p2p_nfc_req_token(wpa_s);
	if (buf == NULL)
		return -1;
...
static int wpa_supplicant_ctrl_iface_p2p_nfc_select_token(
	struct wpa_supplicant *wpa_s, char *cmd, char *reply, size_t max_len)
{
	struct wpabuf *buf;
	int res;

	buf = wpas_p2p_nfc_sel_token(wpa_s, 0);
	if (buf == NULL)
		return -1;
	wpas_p2p_nfc_sel_token_register_timeout(wpa_s);
	res = wpa_snprintf_hex_uppercase(reply, max_len, wpabuf_head(buf),
					 wpabuf_len(buf));
	reply[res++] = '\n';
	reply[res] = '\0';
...
static int wpa_supplicant_ctrl_iface_p2p_nfc_handover_select(
	struct wpa_supplicant *wpa_s, char *cmd, char *reply, size_t max_len)
{
	struct wpabuf *buf;
	int res;

	buf = wpas_p2p_nfc_sel_token(wpa_s, 1);
	if (buf == NULL)
		return -1;
...
static int wpa_supplicant_ctrl_iface_p2p_nfc_tag_read(
	struct wpa_supplicant *wpa_s, char *pos)
{
	size_t len;
	struct wpabuf *buf;
	...
	ret = wpas_p2p_nfc_tag_read(wpa_s, buf, 0);
	wpabuf_free(buf);

	return ret;
}
static int wpa_supplicant_ctrl_iface_p2p_nfc_handover_read(
	struct wpa_supplicant *wpa_s, char *pos)
{
	size_t len;
	struct wpabuf *buf;
	...
	ret = wpas_p2p_nfc_tag_read(wpa_s, buf, 1);
	wpabuf_free(buf);

	return ret;
}

static int wpa_supplicant_ctrl_iface_p2p_set_allocate_ip(
	struct wpa_supplicant *wpa_s, char *cmd)
{
	struct hostapd_data *hapd;
	char *pos, *client_ip, *sub_mask, *go_ip;
	int ret;

	pos = cmd;
	client_ip = os_strstr(pos, "client_ip");
	if (client_ip == NULL) {
		return -1;
	} 
	pos = os_strchr(client_ip, ' ');
	if (pos == NULL)
		return -1;
	*pos++ = '\0';	
	client_ip = os_strchr(client_ip, '=');	
	client_ip++;
	
	sub_mask = os_strstr(pos, "sub_mask");
	if (sub_mask == NULL) {
		return -1;
	} 
	pos = os_strchr(sub_mask, ' ');
	if (pos == NULL)
		return -1;
	*pos++ = '\0';	
	sub_mask = os_strchr(sub_mask, '=');	
	sub_mask++;
	
	go_ip = os_strstr(pos, "go_ip");
	if (go_ip == NULL) {
		return -1;
	} 
	go_ip = os_strchr(go_ip, '=');	
	go_ip++;
		
	wpa_printf(MSG_INFO, "IP ALLOCATED: client_ip=%s, sub_mask=%s, go_ip=%s",
			   client_ip, sub_mask, go_ip);
	wpas_wps_ap_set_alloc_ip(wpa_s, client_ip, sub_mask, go_ip);
			    ...
			    char *buf, size_t buflen)
{
	...
	if (os_strncmp(pos, "pin", 3) == 0) {
		...
		wps_method = WPS_PBC;
#ifdef CONFIG_MTK_P2P_NFC
	} else if (os_strncmp(pos, "oob", 3) == 0) {
		wps_method = WPS_PIN_NFC;
#endif
	} else {
		pin = pos;
		pos = os_strchr(pin, ' ');
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		if (wpas_ctrl_nfc_report_handover(wpa_s, buf + 20))
			reply_len = -1;
#ifdef CONFIG_MTK_WPS_NFC
    } else if (os_strncmp(buf, "WPS_NFC_CFG ", 12) == 0) {
        if (wpa_supplicant_ctrl_iface_wps_nfc_cfg(wpa_s, buf + 12))
            reply_len = -1;
#endif /* CONFIG_MTK_WPS_NFC */
#endif /* CONFIG_WPS_NFC */
		...
		if (p2p_ctrl_remove_client(wpa_s, buf + 18) < 0)
			reply_len = -1;
#ifdef CONFIG_MTK_P2P_NFC
	} else if (os_strncmp(buf, "WPS_NFC_CONFIGURATION_TOKEN ", 28) == 0) {
		reply_len = wpa_supplicant_ctrl_iface_wps_nfc_configuration_token(
			wpa_s, buf + 28, reply, reply_size);
	} else if (os_strncmp(buf, "P2P_NFC_REQUEST_TOKEN", 21) == 0) {
		reply_len = wpa_supplicant_ctrl_iface_p2p_nfc_request_token(
			wpa_s, buf + 14, reply, reply_size);
	} else if (os_strncmp(buf, "P2P_NFC_SELECT_TOKEN", 20) == 0) {
		reply_len = wpa_supplicant_ctrl_iface_p2p_nfc_select_token(
			wpa_s, buf + 14, reply, reply_size);
	} else if (os_strncmp(buf, "P2P_NFC_TAG_READ", 16) == 0) {
		if (wpa_supplicant_ctrl_iface_p2p_nfc_tag_read(
			wpa_s, buf + 17))
			reply_len = -1;
	} else if (os_strncmp(buf, "P2P_NFC_HANDOVER_REQUEST", 24) == 0) {
		reply_len = wpa_supplicant_ctrl_iface_p2p_nfc_request_token(
			wpa_s, buf + 25, reply, reply_size);
	} else if (os_strncmp(buf, "P2P_NFC_HANDOVER_SELECT", 23) == 0) {
		reply_len = wpa_supplicant_ctrl_iface_p2p_nfc_handover_select(
			wpa_s, buf + 24, reply, reply_size);
	} else if (os_strncmp(buf, "P2P_NFC_HANDOVER_READ", 21) == 0) {
		if (wpa_supplicant_ctrl_iface_p2p_nfc_handover_read(
			wpa_s, buf + 22))
			reply_len = -1;
	} else if (os_strncmp(buf, "p2p_set_allocate_ip", 19) == 0) {
		if (wpa_supplicant_ctrl_iface_p2p_set_allocate_ip(wpa_s, buf + 19))
			reply_len = -1;
					       ...
					       char *buf, size_t *resp_len)
{
	...
	static const char * prefix[] = {
		...
		"NFC_RX_HANDOVER_REQ",		
		"WPS_NFC_CONFIGURATION_TOKEN ",
		"P2P_NFC_REQUEST_TOKEN",
		"P2P_NFC_SELECT_TOKEN",
		"P2P_NFC_TAG_READ",
		"P2P_NFC_HANDOVER_REQUEST",
		"P2P_NFC_HANDOVER_SELECT",
		"P2P_NFC_HANDOVER_READ",
		"p2p_set_allocate_ip",			
#endif /* CONFIG_MTK_P2P_NFC */	
		NULL
	};
