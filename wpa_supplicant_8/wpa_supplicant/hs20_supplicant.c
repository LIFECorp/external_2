...
int hs20_sta_enable(struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
	...
	if (ret) {
		...
		wpa_hs20_printf(MSG_DEBUG, "%s", __func__);
	}
...
int wpas_hs20_test_mode(struct wpa_supplicant *wpa_s, enum Hs20CmdType CmdType, const u8 *data)
{
	...
	wpa_hs20_printf(MSG_DEBUG, "[%s] Cmd Type (%d)", __func__, CmdType);
	...
	switch(CmdType){
		...
		case HS20_CMD_ID_SET_BSSID_POOL:
			...
			for(i=0; i<params.hs20_set_bssid_pool.ucNumBssidPool; i++){
				os_memcpy(params.hs20_set_bssid_pool.arBssidPool[i],((struct param_hs20_set_bssid_pool *)data)->arBssidPool[i], ETH_ALEN);				
			}
			break;
		default :
			wpa_hs20_printf(MSG_ERROR, "[%s] Unknown Cmd Type (%d)", __func__, CmdType);
			return -1;
			
	}
	
    return wpa_drv_set_test_mode(wpa_s, (u8 *)&params, 
        sizeof(struct wpa_driver_hs20_data_s));
}
