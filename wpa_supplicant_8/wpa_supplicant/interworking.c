				     ...
				     struct wpa_bss *bss)
{
	...
	if (bss->anqp == NULL || bss->anqp->anqp_3gpp == NULL)
		return -1;
#if 0 //CONFIG_MTK_HS20R1: not sure in KK version. KK without following statement
...
#if defined(CONFIG_MTK_HS20R1) && defined(CONFIG_EAP_SIM_AKA)
	...
	if (set_root_nai(ssid, cred->imsi, prefix) < 0) {
	...
	if (cred->milenage && cred->milenage[0]) {
		...
		sprintf(sim_slot, "%d", cred->pcsc-1);
		if (wpa_config_set_quoted(ssid, "sim_slot", sim_slot) < 0)
			goto fail;
		if (wpa_config_set_quoted(ssid, "pcsc", "rild") < 0)
			goto fail;
#else
...
void interworking_start_fetch_anqp(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf(MSG_DEBUG, "interworking_start_fetch_anqp: REMOVE TEMP NETWORK");
