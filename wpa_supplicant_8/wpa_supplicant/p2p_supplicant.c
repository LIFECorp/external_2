...
static void wps_p2p_nfc_sel_token_timeout(void *eloop_ctx, 
					void *timeout_ctx);
#endif
...
#ifdef CONFIG_MTK_P2P_NFC
	else if (res->wps_method == WPS_PIN_NFC){
		wpas_wps_start_nfc(wpa_s, res->peer_interface_addr, 1);
	}
#endif
	else {
...
#ifdef CONFIG_MTK_P2P_NFC
	else if (params->wps_method == WPS_PIN_NFC) {
		struct p2p_data *p2p = wpa_s->global->p2p;
		 
		wpa_printf(MSG_DEBUG, "P2P: Add WPS PASSWORD TOKEN");
                if(wpa_s->ap_iface){
		    wpa_supplicant_ap_wps_nfc_password_token(wpa_s->ap_iface->bss[0], params->peer_interface_addr,
					  wpabuf_head(p2p->p2p_nfc_peer_pw_token), wpabuf_len(p2p->p2p_nfc_peer_pw_token));
		    wpa_supplicant_ap_set_nfc_password_token(wpa_s->ap_iface->bss[0], wpa_s->conf->wps_nfc_dh_privkey,
...
#ifdef CONFIG_MTK_P2P_NFC
	os_snprintf(oob_role_indication, 24, " oob_role_indication=%d ", info->oob_role_indication);
#endif /* CONFIG_MTK_P2P_NFC */	

#ifdef CONFIG_MTK_P2P_NFC
	wpa_msg_global(wpa_s, MSG_INFO, "%s" MACSTR
		       " p2p_dev_addr=" MACSTR
		       " pri_dev_type=%s name='%s' config_methods=0x%x "
		       "dev_capab=0x%x group_capab=0x%x%s%s" " %s",
		       (info->flags & P2P_DEV_WITH_NFC)? P2P_EVENT_NFC_DEVICE_FOUND : P2P_EVENT_DEVICE_FOUND,
...
#ifdef CONFIG_MTK_P2P_NFC
		       (info->flags & P2P_DEV_WITH_NFC)?  oob_role_indication: ""        
#endif /* CONFIG_MTK_P2P_NFC */	 
		       );
...
#ifdef CONFIG_MTK_P2P_NFC		
		if (p2p->p2p_nfc_select_token == 1) {
			char pw[32 * 2 + 1];

			p2p->p2p_nfc_select_token = 0;
			wpa_s->p2p_wps_method = WPS_PIN_NFC;
			if (wpa_s->conf->wps_nfc_dh_pubkey == NULL ||
				wpa_s->conf->wps_nfc_dh_privkey == NULL ||
				wpa_s->conf->wps_nfc_dev_pw == NULL) {
				wpa_printf(MSG_ERROR, "P2P: NFC DH and PW was not init...");				
				return -1;
			}		  
			wpa_snprintf_hex_uppercase(pw, sizeof(pw),
					   wpabuf_head(wpa_s->conf->wps_nfc_dev_pw),
					   wpabuf_len(wpa_s->conf->wps_nfc_dev_pw));		
			os_snprintf(wpa_s->p2p_pin, sizeof(wpa_s->p2p_pin), "%s",
					pw);
			wpa_msg(wpa_s, MSG_INFO, "P2P NFC: PIN %s", wpa_s->p2p_pin);		
			eloop_cancel_timeout(wps_p2p_nfc_sel_token_timeout, p2p, NULL);
			wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_NFC_GO_INVITED
				" go_iface=" MACSTR, MAC2STR(go_dev_addr));
			goto accept_inv;
		     ...
		     int ht40)
{
	...
	if (wpa_s->force_p2p_go_intent < 16) {
		wpa_printf(MSG_INFO, "Force GO Intent=%d", wpa_s->force_p2p_go_intent);
		go_intent = wpa_s->force_p2p_go_intent;
	}
	...
	if (pin)
		...
		wpa_printf(MSG_DEBUG, "P2P: Randomly generated PIN: %s",
			   wpa_s->p2p_pin);
#ifdef CONFIG_MTK_P2P_NFC			   
	} else if (wps_method == WPS_PIN_NFC) {		
		char pw[32 * 2 + 1];		 
		struct p2p_data *p2p;
		...
		p2p = wpa_s->global->p2p;
		if (p2p->p2p_nfc_handover == 1) {
			wpa_printf(MSG_DEBUG, "P2P: NFC HANDOVER - PIN is no need");			
		}
		//alway enable IP allocate function for P2P NFC
		wpa_printf(MSG_DEBUG, "P2P: NFC ip_alloc_need=%d", wpa_s->ip_alloc_need);		
#endif /* CONFIG_MTK_P2P_NFC */				   
	} else
		wpa_s->p2p_pin[0] = '\0';
...
#endif
#ifdef CONFIG_MTK_P2P_NFC
//Peter-jc.lin This is a temp solution. We need to know Request or Select from Record content 
//WLAN framework
struct wpabuf * wpas_p2p_nfc_req_token(struct wpa_supplicant *wpa_s);
int p2p_nfc_password_valid = 0;
int p2p_nfc_password_len = 0;
u8 p2p_nfc_password[32];
u16 p2p_nfc_dev_pw_id = 0;
int p2p_nfc_add_password_token(struct wpa_supplicant *wpa_s)
{
...
int p2p_nfc_update_password_token(struct wpa_supplicant *wpa_s, u8 *oob_dev_pw,
					 size_t oob_dev_pw_len)
{
	...
	//Replace OOB password token with Select Record
	wpa_printf(MSG_DEBUG, "Update Local Password from Select Record");
	...
	wpa_hexdump(MSG_DEBUG, "WPS: Device Password", dev_pw, dev_pw_len);
	   
	pw = wpabuf_alloc(dev_pw_len);
	if (pw == NULL)
		return -1;
	wpabuf_put_data(pw, dev_pw, dev_pw_len);
		
	if (wpa_s->conf->wps_nfc_dev_pw)	
		wpabuf_free(wpa_s->conf->wps_nfc_dev_pw);
	wpa_s->conf->wps_nfc_dev_pw = pw;	
	wpa_s->conf->wps_nfc_dev_pw_id = id;

	#if 0   
	 if (p2p->p2p_nfc_local_type == P2P_NFC_TYPE_REQ) {
	  	//Replace OOB password token with Select Record
	  	wpa_printf(MSG_DEBUG, "Update Local Password from Select Record");
			pw = wpabuf_alloc(dev_pw_len);
			if (pw == NULL)
			...
			wpa_s->conf->wps_nfc_dev_pw = pw;	
			wpa_s->conf->wps_nfc_dev_pw_id = id;
	  } else if (p2p->p2p_nfc_local_type == P2P_NFC_TYPE_SEL) {
	  	//Override Peer's OOB password token with Local Select Record
	  	//Todo:we need to check length here
	  	wpa_printf(MSG_DEBUG, "Update PEER's Password from Request Record");
	  	os_memcpy(dev_pw, wpabuf_head(wpa_s->conf->wps_nfc_dev_pw), dev_pw_len);
	  	id = wpa_s->conf->wps_nfc_dev_pw_id;
		WPA_PUT_BE16(pw_id, id);
	  }
	 #endif 
	  p2p_nfc_password_len = dev_pw_len;
	  os_memcpy(p2p_nfc_password, dev_pw, p2p_nfc_password_len);	
	  p2p_nfc_dev_pw_id = id;
	  p2p->p2p_nfc_dev_pw_id = id;
	  p2p_nfc_password_valid = 1;
	  
	return 0;
}
int wps_build_nfc_dev_password(struct wpabuf *msg, struct wpa_supplicant *wpa_s, int p2p_nfc_handover)
{
	struct p2p_data *p2p;
	struct wpabuf *pwd_token, *nfc_dev_pw, *nfc_dh_pubkey, *oob_dev_pw;	
	const u8 *pos, *end;
	u16 type, len;	
	
	p2p = wpa_s->global->p2p;
	pwd_token = wps_nfc_token_gen(0, &wpa_s->conf->wps_nfc_dev_pw_id,
				 &wpa_s->conf->wps_nfc_dh_pubkey,
				 &wpa_s->conf->wps_nfc_dh_privkey,
				 &wpa_s->conf->wps_nfc_dev_pw);
	if (pwd_token == NULL) {
		wpa_printf(MSG_ERROR, "WPS: P2P NFC device password generation error");
		return -1;
	}		
	nfc_dev_pw = wpa_s->conf->wps_nfc_dev_pw;
	nfc_dh_pubkey = wpa_s->conf->wps_nfc_dh_pubkey;
	
	oob_dev_pw = wpabuf_alloc(200);
	if (oob_dev_pw == NULL) {
		wpa_printf(MSG_ERROR, "WPS: OOB Device Password Allocate Fail...");
		return -1;
	}
	
    if (p2p_nfc_handover == 1) {
		wpa_printf(MSG_DEBUG, "WPS: OOB for NFC Handover...");
		wpa_s->conf->wps_nfc_dev_pw_id = DEV_PW_NFC_CONNECTION_HANDOVER;
		wps_build_oob_dev_pw(oob_dev_pw, DEV_PW_NFC_CONNECTION_HANDOVER, 
			nfc_dh_pubkey, NULL, 0);
    } else {
		wpa_printf(MSG_DEBUG, "WPS: OOB for NFC Tag...");
		wps_build_oob_dev_pw(oob_dev_pw, wpa_s->conf->wps_nfc_dev_pw_id, 
			nfc_dh_pubkey, wpabuf_head(nfc_dev_pw), wpabuf_len(nfc_dev_pw));

    }
	wpabuf_put_data(msg, wpabuf_head(oob_dev_pw), wpabuf_len(oob_dev_pw)); 

	pos = wpabuf_head(pwd_token);
	end = pos + wpabuf_len(pwd_token);
	while ((end - pos) >= 4) {
		type = WPA_GET_BE16(pos);
		pos += 2;
		len = WPA_GET_BE16(pos);
		pos += 2;
		if (type == ATTR_OOB_DEVICE_PASSWORD){
			//keep oob device password in p2p_data(not include attribute ID and length) 
  		struct wpabuf *pw;	

	  	pw = wpabuf_alloc(len);
			if (pw == NULL)
				return -1;
			wpabuf_put_data(pw, pos, len);
			if (p2p->p2p_nfc_peer_pw_token)	
				wpabuf_free(p2p->p2p_nfc_peer_pw_token);
			p2p->p2p_nfc_peer_pw_token = pw;
			os_memcpy(wpabuf_head(p2p->p2p_nfc_peer_pw_token), 
				P2P_NFC_LOCAL_PUBLIC_KEY_HASH, strlen(P2P_NFC_LOCAL_PUBLIC_KEY_HASH));
  		
				break;
			}
		pos += len;
	}
	wpabuf_free(pwd_token);	
	wpabuf_free(oob_dev_pw);	
	return 0;
}
...
struct wpabuf * wpas_p2p_nfc_req_token(struct wpa_supplicant *wpa_s)
{
	...
	struct wpabuf *p2p_ie, *ret;
	struct wpa_ssid *ssid;	
	u8 oob_role_indication = 0;

	wpa_printf(MSG_DEBUG, "P2P: Building NFC Request Token");
	...
	p2p->p2p_nfc_local_type = P2P_NFC_TYPE_REQ;
	wps_ie = wpabuf_alloc(500);
	p2p_ie =  wpabuf_alloc(200);
	ret = wpabuf_alloc(1000);
	
	if ((wps_ie == NULL) || (ret == NULL) || (p2p_ie == NULL))
		return NULL;

	//WPS INFO
	wpabuf_put_be16(wps_ie, 0);				 
	if (wps_build_manufacturer(dev, wps_ie) ||
	    wps_build_model_name(dev, wps_ie) ||
	    wps_build_model_number(dev, wps_ie) ||
	    wps_build_nfc_dev_password(wps_ie, wpa_s, 1)||
	    wps_build_rf_bands(dev, wps_ie) ||
	    wps_build_serial_number(dev, wps_ie) ||
	    wps_build_uuid_e(wps_ie, uuid) ||
	    wps_build_wfa_ext(wps_ie, req_type == WPS_REQ_ENROLLEE, NULL, 0) 
		) {
		wpabuf_free(wps_ie);
		return NULL;
	}
	WPA_PUT_BE16((u8 *) wpabuf_head(wps_ie), wpabuf_len(wps_ie) - 2);
	wpa_printf(MSG_INFO, "P2P: WPS attributes len=%d", wpabuf_len(wps_ie) - 2);
	p2p->p2p_nfc_dev_pw_id = wpa_s->conf->wps_nfc_dev_pw_id;
	//P2P INFO
	wpabuf_put_be16(p2p_ie, 0);				 
	p2p_buf_add_capability(p2p_ie, p2p->dev_capab &
			       ~P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY, 0);			       			       		       
	//if (p2p->cfg->dev_addr)
	//	p2p_buf_add_device_id(p2p_ie, p2p->cfg->dev_addr);
	p2p_buf_add_device_info(p2p_ie, p2p, NULL);  
	ssid = wpa_s->current_ssid;
	if (ssid) {
		wpa_printf(MSG_DEBUG, "P2P: Group was formated...");
		if(ssid->mode == WPAS_MODE_P2P_GO) {
			oob_role_indication = 0x02;
		} else {
			oob_role_indication = 0x01;			
		}
	}	
	wpa_printf(MSG_INFO, "P2P: P2P Role Indication=%d (2=GO, 1=GC, 0=No group)", oob_role_indication);

	if (p2p->cfg->reg_class && p2p->cfg->channel)
		p2p_buf_add_oob_go_neg_channel(p2p_ie, p2p->cfg->country,
					   p2p->cfg->reg_class,
					   p2p->cfg->channel,
					   oob_role_indication);
	//p2p_buf_add_p2p_version(p2p_ie);
	WPA_PUT_BE16((u8 *) wpabuf_head(p2p_ie), wpabuf_len(p2p_ie) - 2);
	wpa_printf(MSG_INFO, "P2P: P2P attributes len=%d", wpabuf_len(p2p_ie) - 2);

	//Combine WPS INFO and P2P INFO	
	wpabuf_put_buf(ret, wps_ie);
	wpabuf_put_buf(ret, p2p_ie);
...
static void wps_p2p_nfc_sel_token_timeout(void *eloop_ctx, void *timeout_ctx)
{
	struct p2p_data *p2p = eloop_ctx;

	wpa_printf(MSG_DEBUG, "P2P: Select Token timed out - disable Select Token");
	p2p->p2p_nfc_select_token = 0;

}

void wpas_p2p_nfc_sel_token_register_timeout(struct wpa_supplicant *wpa_s)
{
	struct p2p_data *p2p;

	p2p = wpa_s->global->p2p;

	p2p->p2p_nfc_select_token = 1;
	eloop_cancel_timeout(wps_p2p_nfc_sel_token_timeout, p2p, NULL);
	eloop_register_timeout(WPS_PBC_WALK_TIME, 0, wps_p2p_nfc_sel_token_timeout,
			       p2p, NULL);
}

struct wpabuf * wpas_p2p_nfc_sel_token(struct wpa_supplicant *wpa_s, int p2p_nfc_handover)
{
	struct wps_device_data *dev;
	const u8 *uuid;
	enum wps_request_type req_type;
	...
	u8 oob_role_indication = 0;
	int freq;
	char oob_neg_country[3];
	u8 oob_neg_reg_class = 0;
	u8 oob_neg_channel = 0;

	wpa_printf(MSG_DEBUG, "P2P: Building NFC Select Token");
	...
	p2p->p2p_nfc_local_type = P2P_NFC_TYPE_SEL;
	wps_ie = wpabuf_alloc(500);
	p2p_ie =  wpabuf_alloc(200);
	ret = wpabuf_alloc(1000);
	...
	if (wps_build_manufacturer(dev, wps_ie) ||
	    wps_build_model_name(dev, wps_ie) ||
	    wps_build_model_number(dev, wps_ie) ||
	    wps_build_nfc_dev_password(wps_ie, wpa_s, p2p_nfc_handover)||
	    wps_build_rf_bands(dev, wps_ie) ||
	    wps_build_serial_number(dev, wps_ie) ||
	    wps_build_uuid_e(wps_ie, uuid) ||
	    wps_build_wfa_ext(wps_ie, req_type == WPS_REQ_ENROLLEE, NULL, 0)
	...
	p2p_buf_add_device_info(p2p_ie, p2p, NULL);
	os_memcpy(p2p->intended_addr, p2p->cfg->dev_addr, ETH_ALEN);

	ssid = wpa_s->current_ssid;
	if (ssid && (wpa_s->wpa_state >= WPA_ASSOCIATED)) {
		wpa_printf(MSG_DEBUG, "P2P: Group was formated...");
		if(ssid->mode == WPAS_MODE_P2P_GO) {
			oob_role_indication = 0x02;
		} else {
		...
		freq = wpa_s->current_bss ? wpa_s->current_bss->freq :
		(int) wpa_s->assoc_freq;
		wpa_printf(MSG_INFO, "P2P: Group formated freq=%d go_dev_addr="MACSTR,
			freq, MAC2STR(wpa_s->go_dev_addr));

		p2p_freq_to_channel(ssid->frequency,
			&oob_neg_reg_class, &oob_neg_channel);
	}	

	//wpabuf_put_u8(p2p_ie, P2P_ATTR_GROUP_INFO);
	//wpabuf_put_le16(p2p_ie, 0);
	//Add GROUP ID if device is GC

	wpa_printf(MSG_INFO, "P2P: P2P Role Indication=%d (2=GO, 1=GC, 0=No group)", oob_role_indication);
	if (oob_role_indication == 0x01) {
		p2p_buf_add_group_id(p2p_ie, wpa_s->go_dev_addr, ssid->ssid, ssid->ssid_len);	
		p2p_buf_add_oob_go_neg_channel(p2p_ie, p2p->cfg->country,
					   oob_neg_reg_class,
					   oob_neg_channel,
					   oob_role_indication);
	} else if (oob_role_indication == 0x02) {
		wpabuf_put_u8(p2p_ie, P2P_ATTR_GROUP_INFO);
		wpabuf_put_le16(p2p_ie, 0);
		wpa_printf(MSG_INFO, "P2P: P2P GO include Group Info");
		p2p_buf_add_group_id(p2p_ie, p2p->cfg->dev_addr, ssid->ssid, ssid->ssid_len);	
		p2p_buf_add_oob_go_neg_channel(p2p_ie, p2p->cfg->country,
					   oob_neg_reg_class,
					   oob_neg_channel,
					   oob_role_indication);
	} else if (p2p->cfg->reg_class && p2p->cfg->channel) {
		p2p_buf_add_oob_go_neg_channel(p2p_ie, p2p->cfg->country,
					   p2p->cfg->reg_class,
					   p2p->cfg->channel,
					   oob_role_indication);
	}
	if (oob_role_indication == 0x02) {
		if(wpa_s->ap_iface){
			wpa_supplicant_ap_set_nfc_password_token(wpa_s->ap_iface->bss[0], wpa_s->conf->wps_nfc_dh_privkey,
				wpa_s->conf->wps_nfc_dh_pubkey);
		}
	...
	wpa_s->ip_alloc_need = 1;
	wpa_printf(MSG_INFO, "Enable Client IP Allocate");
...
static int wpas_p2p_nfc_tag_process(struct wpa_supplicant *wpa_s,
				    const struct wpabuf *p2p_ndef)
{
	const struct wpabuf *p2p_nfc;
	struct p2p_data *p2p;

    p2p = wpa_s->global->p2p;
	p2p_nfc = ndef_parse_wifi_p2p(p2p_ndef);
	if (p2p_nfc == NULL) {
		wpa_printf(MSG_DEBUG, "P2P: Could not parse NDEF");
		return -1;
	}  
	
  if (p2p_add_device_nfc(wpa_s, p2p, p2p_nfc)) {
		wpa_printf(MSG_DEBUG, "P2P: Could not add device from NFC");
		return -1;
	}
	return 0;
}
...
int wpas_p2p_nfc_tag_read(struct wpa_supplicant *wpa_s,
			  const struct wpabuf *data, int p2p_nfc_handover)
{
	struct p2p_data *p2p;
	int ret;
	...
	p2p = wpa_s->global->p2p;
	p2p->p2p_nfc_handover = p2p_nfc_handover;

	ret = wpas_p2p_nfc_tag_process(wpa_s, data);
	wpa_s->ip_alloc_need = 1;
	wpa_printf(MSG_INFO, "Enable Client IP Allocate");

	return ret;
}
int wpas_p2p_go_wps_nfc(struct wpa_supplicant *wpa_s)
{
	struct p2p_data *p2p;

	p2p = wpa_s->global->p2p;

	return wpa_supplicant_ap_wps_nfc_password_token(wpa_s->ap_iface->bss[0], NULL,
		  wpabuf_head(p2p->p2p_nfc_peer_pw_token), wpabuf_len(p2p->p2p_nfc_peer_pw_token));
