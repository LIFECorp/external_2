...
static int wpa_cli_cmd_wps_nfc_cfg(struct wpa_ctrl *ctrl, int argc,
                   char *argv[])
{
    char cmd[256];
    int res;
    ...
    if (argc != 2) {
        printf("Invalid WPS_NFC_CFG command: needs two arguments\n"
               "variable name, and value)\n");
        return -1;
    }

    res = os_snprintf(cmd, sizeof(cmd), "WPS_NFC_CFG %s %s",
              argv[0], argv[1]);

    if (res < 0 || (size_t) res >= sizeof(cmd) - 1) {
        printf("Too long WPS_NFC_CFG command.\n");
        return -1;
    }
    return wpa_ctrl_command(ctrl, cmd);
}
...
static int wpa_cli_cmd_wps_nfc_configuration_token(struct wpa_ctrl *ctrl, int argc,
				     char *argv[])
{
	return wpa_cli_cmd(ctrl, "WPS_NFC_CONFIGURATION_TOKEN", 1, argc, argv);
}
static int wpa_cli_cmd_p2p_nfc_handover_request(struct wpa_ctrl *ctrl, int argc,
				     char *argv[])
{
	return wpa_cli_cmd(ctrl, "P2P_NFC_HANDOVER_REQUEST", 0, argc, argv);
}

static int wpa_cli_cmd_p2p_nfc_handover_select(struct wpa_ctrl *ctrl, int argc,
				     char *argv[])
{
	return wpa_cli_cmd(ctrl, "P2P_NFC_HANDOVER_SELECT", 0, argc, argv);
}
static int wpa_cli_cmd_p2p_nfc_handover_read(struct wpa_ctrl *ctrl, int argc,
					char *argv[])
{
	int ret;
	char *buf;
	...
	if (argc != 1) {
		printf("Invalid 'p2p_nfc_handover_read' command - one argument "
		       "is required.\n");
		return -1;
	}

	buflen = 24 + os_strlen(argv[0]);
	buf = os_malloc(buflen);
	if (buf == NULL)
		return -1;
	os_snprintf(buf, buflen, "P2P_NFC_HANDOVER_READ %s", argv[0]);
...
static int wpa_cli_cmd_p2p_nfc_request_token(struct wpa_ctrl *ctrl, int argc,
				     char *argv[])
{
	return wpa_cli_cmd(ctrl, "P2P_NFC_REQUEST_TOKEN", 0, argc, argv);
}

static int wpa_cli_cmd_p2p_nfc_select_token(struct wpa_ctrl *ctrl, int argc,
				     char *argv[])
{
	return wpa_cli_cmd(ctrl, "P2P_NFC_SELECT_TOKEN", 0, argc, argv);
}

static int wpa_cli_cmd_p2p_nfc_tag_read(struct wpa_ctrl *ctrl, int argc,
					char *argv[])
{
	int ret;
	char *buf;
	...
	if (argc != 1) {
		printf("Invalid 'p2p_nfc_tag_read' command - one argument "
		       "is required.\n");
		return -1;
	}
	...
	if (buf == NULL)
		return -1;
	os_snprintf(buf, buflen, "P2P_NFC_TAG_READ %s", argv[0]);
...
static int wpa_cli_cmd_p2p_set_allocate_ip(struct wpa_ctrl *ctrl,
				  int argc, char *argv[])
{
	return wpa_cli_cmd(ctrl, "p2p_set_allocate_ip", 3, argc, argv);
}
...
static struct wpa_cli_cmd wpa_cli_commands[] = {
	...
	{ "nfc_report_handover", wpa_cli_cmd_nfc_report_handover, NULL,
	  ...
	  "NFC handover" },
#ifdef CONFIG_MTK_WPS_NFC
	{ "wps_nfc_cfg", wpa_cli_cmd_wps_nfc_cfg, NULL,
	  cli_cmd_flag_sensitive,
	  "pubkey = use different public key" },
#endif /* CONFIG_MTK_WPS_NFC */
#endif /* CONFIG_WPS_NFC */
...
#ifdef CONFIG_MTK_P2P_NFC
	{ "wps_nfc_configuration_token", wpa_cli_cmd_wps_nfc_configuration_token, NULL,
	  cli_cmd_flag_none,
	  "create WPS NFC configuration token for P2P GO" },
	{ "p2p_nfc_handover_request", wpa_cli_cmd_p2p_nfc_handover_request, NULL,
	  cli_cmd_flag_none,
	  "create P2P NFC Handover Request" },
	{ "p2p_nfc_handover_select", wpa_cli_cmd_p2p_nfc_handover_select, NULL,
	  cli_cmd_flag_none,
	  "create P2P NFC Handover Select" },
	{ "p2p_nfc_handover_read", wpa_cli_cmd_p2p_nfc_handover_read, NULL,
	  cli_cmd_flag_sensitive,
	  "<hexdump of payload> = report read NFC handover with P2P data" },
	{ "p2p_nfc_request_token", wpa_cli_cmd_p2p_nfc_request_token, NULL,
	  cli_cmd_flag_none,
	  "create P2P NFC request token" },
	{ "p2p_nfc_select_token", wpa_cli_cmd_p2p_nfc_select_token, NULL,
	  cli_cmd_flag_none,
	  "create P2P NFC select token" },
	{ "p2p_nfc_tag_read", wpa_cli_cmd_p2p_nfc_tag_read, NULL,
	  cli_cmd_flag_sensitive,
	  "<hexdump of payload> = report read NFC tag with P2P data" },
	{ "p2p_set_allocate_ip", wpa_cli_cmd_p2p_set_allocate_ip, NULL,
	  cli_cmd_flag_sensitive,
	  "GO set GC's IP if P2P connection is trigger by NFC" },
