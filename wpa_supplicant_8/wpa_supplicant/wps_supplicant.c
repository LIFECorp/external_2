...
static void wpa_supplicant_wps_event_success(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->current_ssid && wpa_s->wps_er == NULL)
#else
...
#ifdef CONFIG_MTK_WPS_NFC
int wpas_wps_start_nfc(struct wpa_supplicant *wpa_s, const u8 *bssid, int p2p_group)
{
	struct wps_context *wps = wpa_s->wps;
	char pw[32 * 2 + 1];
    ...
    wpa_hexdump_buf_key(MSG_DEBUG, "Maggie: DH Private Key(org)", wps->dh_privkey);
	wpa_hexdump_buf(MSG_DEBUG, "Maggie: DH own Public Key(org)", wps->dh_pubkey);
    if(wpa_s->fgpubkey == 1) {
        wpa_printf(MSG_DEBUG, "Maggie: Generate new DH keys");
        wpabuf_free(wps->dh_pubkey);
		wpabuf_free(wps->dh_privkey);
		wps->dh_privkey = NULL;
		wps->dh_pubkey = NULL;
		...
		if (wps->dh_ctx == NULL || wps->dh_pubkey == NULL) {
			wpa_printf(MSG_ERROR, "Maggie: Failed to initialize "
    ...
    wpa_hexdump_buf_key(MSG_DEBUG, "Maggie: DH Private Key", wps->dh_privkey);
	wpa_hexdump_buf(MSG_DEBUG, "Maggie: DH own Public Key", wps->dh_pubkey);

	wpa_snprintf_hex_uppercase(pw, sizeof(pw),
				   wpabuf_head(wpa_s->conf->wps_nfc_dev_pw),
				   wpabuf_len(wpa_s->conf->wps_nfc_dev_pw));
	
	wpa_printf(MSG_INFO,"wpas_wps_start_nfc dev_pw_id = %d", wpa_s->conf->wps_nfc_dev_pw_id);
	return wpas_wps_start_pin(wpa_s, bssid, pw, p2p_group,
				  wpa_s->conf->wps_nfc_dev_pw_id);
}
#else
...
#ifdef CONFIG_MTK_P2P_NFC
static int wpas_wps_add_nfc_password_token_go(struct wpa_supplicant *wpa_s,
					   struct wps_parse_attr *attr)
{
				    ...
				    const struct wpabuf *wps)
{
	...
	if (attr.oob_dev_password){
		if (wpa_s->ap_iface){
			return wpas_wps_add_nfc_password_token_go(wpa_s, &attr);
        }
        else{
...
{
#ifndef CONFIG_MTK_WPS_NFC
	if (cr)
		return ndef_build_wifi_hc(1);
	return ndef_build_wifi_hr();
#else
    if(wpa_s->wps->uuid){
        return wps_nfc_handover_req_gen(wpa_s->wps->uuid, 
                     &wpa_s->conf->wps_nfc_dev_pw_id,
    				 &wpa_s->conf->wps_nfc_dh_pubkey,
    				 &wpa_s->conf->wps_nfc_dh_privkey,
    				 &wpa_s->conf->wps_nfc_dev_pw);  
        ...
        wpa_printf(MSG_ERROR, "WPS: Could not generate handover request message.");
        return NULL;
    }
					  ...
					  int ndef, int cr, const char *uuid)
{
    ...
    if (wpa_s->assoc_freq == 0) {
        wpa_printf(MSG_ERROR, "WPS: Not associated with any AP");
        return NULL;
    } else if (wpa_s->assoc_freq >= 2412 && wpa_s->assoc_freq <= 2472) {
        ap_channel = (wpa_s->assoc_freq - 2407) / 5;
    } else if ((wpa_s->assoc_freq >= 5180 && wpa_s->assoc_freq <= 5240) ||
        (wpa_s->assoc_freq >= 5745 && wpa_s->assoc_freq <= 5825)) {
            ap_channel = (wpa_s->assoc_freq - 5000) / 5;
    } else {
	wpa_printf(MSG_ERROR, "WPS: Unsupported AP mode frequency: %d MHz",
			  wpa_s->assoc_freq);
        return NULL;
    }
    
    sel = wps_nfc_handover_sel_gen(wpa_s->current_ssid->ssid, 
                  wpa_s->current_ssid->ssid_len, 
                  &wpa_s->wps->dev, ap_channel, wpa_s->bssid,
                 &wpa_s->conf->wps_nfc_dev_pw_id,
				 &wpa_s->conf->wps_nfc_dh_pubkey,
				 &wpa_s->conf->wps_nfc_dh_privkey,
				 &wpa_s->conf->wps_nfc_dev_pw);
    ...
    if (sel != NULL) {
        if (wpa_s->ap_iface){ 
            ...
            wps_er_nfc_password_token(wpa_s->wps, wpa_s->conf->wps_nfc_dh_privkey,
			wpa_s->conf->wps_nfc_dh_pubkey);
        }
    }
				 ...
				 const struct wpabuf *data)
{
	...
	pos = wpabuf_head_u8(wps);
	end = pos + wpabuf_len(wps);	
	attr_len = WPA_GET_BE16(pos);
	wpa_printf(MSG_INFO, "WPS: Handover REQ WPS len=%d", attr_len);
	if (attr_len > 500) {
		wpa_printf(MSG_ERROR, "WPS: Unreasonable WPS len=%d", attr_len);
		return -1;
	}
		    
	buf = wpabuf_alloc(attr_len);
	if (buf == NULL)
		return -1;
	wpabuf_put_data(buf, pos + 2, attr_len);		
	ret = wpas_wps_nfc_tag_process(wpa_s, buf);
	wpabuf_free(wps);
	wpabuf_free(buf);
